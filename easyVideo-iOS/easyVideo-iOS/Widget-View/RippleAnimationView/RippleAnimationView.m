...
- (CALayer *)pulsingLayer:(CGRect)rect animation:(CAAnimationGroup *)animationGroup {
    ...
    if (_animationType == AnimationTypeWithBackground) {
        pulsingLayer.backgroundColor = ColorWithAlpha(127, 157, 201, 0.5).CGColor;
        pulsingLayer.borderWidth = 0.5;
    } else {
        pulsingLayer.borderWidth = 1.f;
    }
    
    pulsingLayer.borderColor = ColorWithAlpha(127, 157, 201, 0.5).CGColor;
...
- (CAKeyframeAnimation *)backgroundColorAnimation {
    ...
    backgroundColorAnimation.values = @[(__bridge id)ColorWithAlpha(127, 157, 201, 0.5).CGColor,
                                        (__bridge id)ColorWithAlpha(82, 128, 186, 0.5).CGColor,
                                        (__bridge id)ColorWithAlpha(44, 104, 164, 0.5).CGColor,
                                        (__bridge id)ColorWithAlpha(44, 104, 164, 0).CGColor];
...
- (CAKeyframeAnimation *)borderColorAnimation {
    ...
    borderColorAnimation.values = @[(__bridge id)ColorWithAlpha(127, 157, 201, 0.5).CGColor,
                                    (__bridge id)ColorWithAlpha(82, 128, 186, 0.5).CGColor,
                                    (__bridge id)ColorWithAlpha(44, 104, 164, 0.5).CGColor,
                                    (__bridge id)ColorWithAlpha(44, 104, 164, 0).CGColor];
