...
//数据库
static NSString * const sqliteName = @"EVUserIdTab.sqlite";
@implementation EVUserIdManager

@synthesize managedObjectContext = _managedObjectContext;
@synthesize managedObjectModel = _managedObjectModel;
@synthesize persistentStoreCoordinator = _persistentStoreCoordinator;

static EVUserIdManager *coreDataManager = nil;
+ (instancetype)sharedInstance
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coreDataManager = [[EVUserIdManager alloc] init];
    });
...
- (void)insertNewEntity:(EMGroupMemberInfo *)model success:(void (^__nullable)(void))success fail:(void (^__nullable)(NSError *))fail {
    NSDictionary *dict = [Utils getObjectData:model];
...
- (void)updateEntity:(void (^__nullable)(void))success fail:(void (^__nullable)(NSError *))fail {
    NSError *error = nil;
    [self.managedObjectContext save:&error];
    if (error) {
        if (fail) {
