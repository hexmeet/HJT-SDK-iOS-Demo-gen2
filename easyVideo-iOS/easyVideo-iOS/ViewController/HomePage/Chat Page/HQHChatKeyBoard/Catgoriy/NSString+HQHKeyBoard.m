...
@implementation NSString (HQHKeyBoard)
...
+ (void)deleteEmtionString:(UITextView *)textView isSystem:(BOOL)isSystem {
    ...
    if (range.length > 0) {
        ...
        for (HQHGroupEmojiModel *groupModel in HQHChatKeyBoardManager.sharedManager.groupEmojis) {
            if (groupModel.type != HQHChatEmojiSmall) {
                continue;
            }
            
            for (HQHEmojiModel *model in groupModel.emojis) {
    ...
    for (NSInteger i = resultArray.count-1; i >= 0; i--) {
        ...
        for (HQHGroupEmojiModel *groupModel in HQHChatKeyBoardManager.sharedManager.groupEmojis) {
            ...
            for (int i = 0; i < groupModel.emojis.count; i++) {
                HQHEmojiModel *model = groupModel.emojis[i];
