...
@interface HQHChatKeyBoardView () <UITextViewDelegate, HQHChatEmojiViewDelegate, HQHChatMoreViewDelegate>
...
@property (nonatomic, strong) HQHKeyBoardTextView *textView;
@property (nonatomic, strong) UIView *bottomView;
//表情 视图
@property (nonatomic, strong) HQHChatEmojiView *emojiView;
//更多 视图
@property (nonatomic, strong) HQHChatMoreView *moreView;

//当前键盘状态
@property (nonatomic, assign) HQHKeyBoardState currentState;
//textView宽度
...
//录音动画视图
@property (nonatomic, strong) HQHVoiceAnimationView *voiceView;

@end

@implementation HQHChatKeyBoardView
...
- (instancetype)initWithNavigationBarTranslucent:(BOOL)translucent
{
    ...
    if (self) {
        ...
        _currentState = HQHKeyBoardStateNormal;
        _showFace = YES;
        _showVoice = YES;
        _showMore = YES;
...
- (void)textViewDidBeginEditing:(UITextView *)textView {
    self.currentState = HQHKeyBoardStateKeyBoard;
}
...
- (void)chatEmojiViewDidSelectEmojiWithContent:(HQHEmojiModel *)emoji {
    ...
    if ([emoji.title isEqualToString:@"delete"]) {
        ...
        indexCursor = indexCursor + emoji.title.length;
        
        NSRange ra = {indexCursor,0};
        [self.textView setSelectedRange:ra];
...
- (void)chatEmojiViewDidSelectFaceWithContent:(HQHFaceModel *)model {
...
- (void)pressVoiceButton:(UIButton *)sender {
    ...
    if (sender.selected) {
        self.currentState = HQHKeyBoardStateVoice;
    }else {
        self.currentState = HQHKeyBoardStateKeyBoard;
    }
}
...
- (void)pressFaceButton:(UIButton *)sender {
    ...
    sender.selected = !sender.selected;
    if (sender.selected) {
        self.currentState = HQHKeyBoardStateFace;
...
- (void)pressMoreButton:(UIButton *)sender {
    ...
    sender.selected = !sender.selected;
    if (sender.selected) {
        self.currentState = HQHKeyBoardStateMore;
        if (![self.bottomView.subviews containsObject:self.moreView]) {
...
- (void)beginRecordVoice:(UIButton *)sender {
    ...
    self.voiceView.state = HQHVoiceNormal;
    [HQHAudioRecorder.shared startRecord];
    
}

- (void)endRecordVoice:(UIButton *)sender {
    if ([UIApplication.sharedApplication.keyWindow.subviews containsObject:self.voiceView]) {
        self.voiceView.state = HQHVoiceFinished;
        [self.voiceView removeFromSuperview];
    }
    NSDictionary *recordInfo = [HQHAudioRecorder.shared stopRecord];
...
- (void)cancelRecordVoice:(UIButton *)sender {
    if ([UIApplication.sharedApplication.keyWindow.subviews containsObject:self.voiceView]) {
        self.voiceView.state = HQHVoiceCancel;
        [self.voiceView removeFromSuperview];
    }
    [HQHAudioRecorder.shared cancelRecording];
}

- (void)RemindDragExit:(UIButton *)sender {
    
    self.voiceView.state = HQHVoiceWillCancel;
}
...
- (void)keyboardWillChange:(NSNotification *)noti {
    ...
    if(noti.name == UIKeyboardWillHideNotification){
        ...
        switch (self.currentState) {
            case HQHKeyBoardStateNormal:
            case HQHKeyBoardStateVoice:
            case HQHKeyBoardStateKeyBoard:
            {
                ...
                break;
            case HQHKeyBoardStateFace:
            case HQHKeyBoardStateMore:
            {
                //显示表情及更多
...
- (void)hideBottomView {
    if (self.currentState == HQHKeyBoardStateFace ||
        self.currentState == HQHKeyBoardStateMore) {
        self.currentState = HQHKeyBoardStateNormal;
...
- (void)setCurrentState:(HQHKeyBoardState)currentState {
...
- (HQHKeyBoardTextView *)textView {
    if (!_textView) {
        _textView = [HQHKeyBoardTextView new];
        _textView.delegate = self;
        _textView.returnKeyType = UIReturnKeySend;
        _textView.placeHolder = NSLocalizedString(@"chat.textview.placeholder", @"说点什么吧");
    }
    return _textView;
}
...
- (HQHChatEmojiView *)emojiView {
    if (!_emojiView) {
        _emojiView = [[HQHChatEmojiView alloc] initWithFrame:CGRectMake(0, 0, self.width, kBottomViewHeight)];
        _emojiView.delegate = self;
    }
    return _emojiView;
}

- (HQHChatMoreView *)moreView {
    if (!_moreView) {
        _moreView = [[HQHChatMoreView alloc] initWithFrame:CGRectMake(0, 0, self.width, kBottomViewHeight)];
        _moreView.delegate = self;
    }
    return _moreView;
}

- (HQHVoiceAnimationView *)voiceView {
    if (!_voiceView) {
        _voiceView = [[HQHVoiceAnimationView alloc] initWithFrame:CGRectMake(SCREEN_Width/2.f-160.f/2.f, SCREEN_Height/2.f-180.f/2.f, 160.f, 180.f)];
    }
    return _voiceView;
}
